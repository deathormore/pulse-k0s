...
![Лог успешного создания и загрузки backup](images/evidence/04-backup-success-log.png)

**Что показано:** скрипт создаёт PostgreSQL dump, загружает его в `postgres/` бакета, проверяет объект через S3 API и только после этого сообщает об успешном завершении.

![Загруженный dump в Yandex Object Storage](images/evidence/04-backup-object-in-s3.png)

**Что показано:** в Object Storage присутствует тот же файл `pulse-2026-09-15T21-50-49Z.dump` размером 30,04 КБ, который указан в финальной строке backup-скрипта.

Результат подтверждается сразу на четырёх уровнях:

- в разделе 3 backup Job завершены со статусом `Complete`;
- журнал показывает создание, отправку и проверку объекта;
- имя файла в журнале совпадает с объектом в Yandex Object Storage;
- в разделе 6 Grafana показывает `Backups Stored in S3 = 2`.

**Что подтверждает:** backup-задача не просто запускается по расписанию — dump фактически появляется во внешнем бакете, проверяется через S3 API, а общее количество сохранённых файлов независимо отображается в мониторинге.
